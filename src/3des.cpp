...
int McbDoDES2(unsigned char * lpIn, unsigned long cbIn, unsigned char Key1[8],
    unsigned char Key2[8], unsigned char Key3[8], unsigned char * lpOut, unsigned long * pcbOut,
...
{        
    ...
    */
    ...
    if (nEncNo > 1)
    {
        ...
        if (nEncNo > 2)		// added by cyb
        	McbCreateSubKeys(Key3, SubKeys[2]);
    }
            ...
            for(n = 0; n < (unsigned int)nEncNo; n++)
            {
                ...
                */    // updated by cyb
                McbEncode64Bits(in, lpOut, 
                    (const unsigned char (*)[6])SubKeys[n%3], nEncryptMode);
...
*/
/*
int TriDes2Keys(unsigned char* buffin, unsigned long inlen, unsigned char key1[8], 
		unsigned char key2[8], unsigned char* buffout, unsigned long* outlen,
		int enc, int cbc)
{
	return McbDoDES2(buffin, inlen, key1, key2, key1, buffout, outlen, enc, 0, 3, cbc);
}
*/
		...
		unsigned long* outlen, int enc, int cbc)
{
	if (enc == 0)		//Ω‚√‹
	{
		return McbDoDES2(buffin, inlen, key3, key2, key1, buffout, outlen, enc, 0, 3, cbc);
	}
	return McbDoDES2(buffin, inlen, key1, key2, key3, buffout, outlen, enc, 0, 3, cbc);
}
